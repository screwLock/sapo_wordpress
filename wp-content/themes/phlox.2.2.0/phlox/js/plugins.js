...
;( function( $, window, document, undefined ) {
    ...
    $.extend( Plugin.prototype, {
        init: function() {
            ...
            this._targetsNum = this.$targets.length;
            this.$normalCols  = this.$element.find('.' + this.settings.columns ).not('.' + this.settings.shortClass);
            this.$shortCols   = this.$element.find('.' + this.settings.shortClass);
            ...
            this._checkColumns(this.$normalCols, this.$shortCols);

            this.$element.find('img').imagesLoaded().progress( function( imgLoad , img ){
                self._checkColumns(self.$normalCols, self.$shortCols);
            });
        ...
        _checkColumns: function( $normalCols, $shortCols ){
            var self           = this ,
                shortColBotOffset,
                transform,
                colProp,
                ...
                var shortCol = [].reduce.call($shortCols, function( prev, cur) {
                    return $(prev).height() < $(cur).height() ? prev : cur;
                });

                shortColBotOffset = $(shortCol).height() + $(shortCol).offset().top;

                $(shortCol).attr('data-parallax-transform', 0);
                
                $normalCols.each( function( index, el ){
                    transform = ( $(el).height() + $(el).offset().top )- shortColBotOffset ;
                    colProp   = $(el).height() / shortColBotOffset ;
                    $(el).attr('data-col-prop', colProp);
                    $(el).attr('data-parallax-transform', transform);

                    if ( transform > maxTransform || maxTransform == null) {
                        maxTransform = transform;
                    }   
                });
                
                this.maxTransform = maxTransform *  this.direction;

        },


        _setPosition: function( $cols, maxTransform ){
            ...
            var isOnViewport   = this._isOnViewport( this.settings.yVisible ),
                ...
                transformValue = this.settings.speed * scrollValue * this.direction ;
            ...
            if ( scrollValue <= 0 ){
                ...
                if( isOnViewport ){
                    ...
                    $($cols).each(function (index, el) {
                        var colTransform = transformValue * $(el).data('col-prop');
                        
                        if ( Math.abs(colTransform) < Math.abs( maxTransform ) ) { 
                            el.style[window._jcsspfx + 'Transform'] = 'translateY(' +  colTransform + 'px)';
                        } else {
         ...
         */
        update: function() {
            this._setPosition( this.$normalCols, this.maxTransform );
        }
