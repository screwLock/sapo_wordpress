...
function auxin_the_post_thumbnail( $post_id = null, $width = null , $height = null, $crop = null , $quality = 100, $upscale = false ) {
    ...
    function auxin_get_the_responsive_attachment( $attachment_id = null, $args = array() ) {
        ...
        $defaults = array(
            'quality'         => 100,
            'attr'            => '',
            'preload_off'     => false, // Enable this option to entirely turn off any preload for this image
            ...
            'add_hw'          => true,
            'add_ratio'       => true,
            'image_sizes'     => 'auto', // (sizes)
            'srcset_sizes'    => 'auto', // (srcset) automatically calculate the image sizes based on the 'size' param, OR 'wp' generates image srcs based on WP default image sizes
            'original_src'    => true
        );
        ...
        if( ! $preload_off && $preload_allowed && 'auto' === $image_sizes ){
        ...
        } elseif( ( ! $preload_allowed || $preload_off ) && 'auto' === $image_sizes ) {
            $attr['sizes']  = auxin_generate_image_sizes( array(
                    ...
                    array( 'min' => '', 'max' => ''      , 'width' => $dimensions['width'].'px'  )
                )
            );
        }

        // display a blurred preview of the main image
        if ( $preload_allowed && ! $preload_off ) {
